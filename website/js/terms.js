...
<p>类比："machine speed debt" 指 AI 加速产生的低质量代码累积。</p>`,
  ...
  },
  {
    ...
    longDesc: `<p>2025-06-17，Andrej Karpathy 在 AI Startup School 演讲《Software Is Changing (Again)》中提出 <strong>Software 3.0</strong>：LLM 本身成为可编程的"计算机"，开发者用自然语言——prompts、示例、上下文——指导模型完成软件任务。</p>
<p><strong>演进脉络：</strong>Software 1.0（手写代码）→ Software 2.0（用数据训练出权重）→ Software 3.0（用自然语言编程 LLM）。</p>
<p><strong>与本站的关系：</strong>Software 3.0 是 vibe coding 与 agentic engineering 所处的更大历史框架。本词条为社区提案，处于待验证（pending）状态。</p>`,
    enShortDesc: "Karpathy's 2025 framing: software built by steering LLMs with natural language — the successor to Software 1.0 (hand-written code) and Software 2.0 (learned weights).",
    enLongDesc: `<p>On 2025-06-17, <strong>Andrej Karpathy</strong> laid out <strong>Software 3.0</strong> in his AI Startup School talk "<em>Software Is Changing (Again)</em>": the LLM itself becomes the programmable computer, and developers steer it with natural language — prompts, examples, and context — instead of writing all the code by hand.</p>
<p><strong>The progression:</strong> Software 1.0 (hand-written code) → Software 2.0 (weights learned from data) → Software 3.0 (LLMs programmed in natural language).</p>
<p><strong>Where it sits here:</strong> Software 3.0 is the bigger historical frame that vibe coding and agentic engineering live in. This entry is <strong>pending</strong> — proposed by the community, awaiting verification before promotion.</p>`,
    related: ["vibe-coding","agentic-programming","agentic-engineering","llm"],
...
<p><strong>关键特性：</strong>项目根 CLAUDE.md 在 compaction 后<strong>会重新读取</strong>，而非丢失。</p>`,
    ...
    examples: [
      ...
      {
        "code": "# CLAUDE.md(官方 memory 文档:与 AGENTS.md 共存写法)\n# 首行 @import 使两个工具读同一份指令\n#\n# @AGENTS.md\n#\n# ## Claude Code\n#\n# Use plan mode for changes under `src/billing/`.",
...
<p><strong>代表：</strong>Cohere Rerank 3 / BGE Reranker / ColBERT</p>`,
    ...
    quotes: [
      {
        "text": "SentenceTransformers also supports to load Cross-Encoders for sentence pair scoring and sentence pair classification tasks. In contrast, for a Cross-Encoder, we pass both sentences simultaneously to the Transformer network. It produces then an output value between 0 and 1 indicating the similarity of the input sentence pair.",
        "cite": "sentence-transformers 官方文档《Cross-Encoders》, 2026-09-04 检索"
      },
      {
        "text": "Given a `query` and a list of `documents`, Rerank indexes the documents from most to least semantically relevant to the query.",
        "cite": "Cohere 官方文档《An Overview of Cohere's Rerank Model》, 2026-09-04 检索"
      },
      {
        "text": "In this paper, we describe a simple re-implementation of BERT for query-based passage re-ranking.",
        "cite": "Rodrigo Nogueira, Kyunghyun Cho, arXiv:1901.04085 摘要, 2019-01"
      }
    ],
    seeAlso: [
      {
      ...
      {
        "name": "Cross-Encoders — Sentence Transformers 文档",
        "url": "https://www.sbert.net/docs/cross_encoder/usage/usage.html",
        "lastVerified": "2026-09-04"
      },
      {
        "name": "An Overview of Cohere's Rerank Model — Cohere 文档",
    ...
    examples: [
      ...
      {
        "code": "from sentence_transformers.cross_encoder import CrossEncoder\nmodel = CrossEncoder(\"cross-encoder/ms-marco-MiniLM-L6-v2\")\nscores = model.predict([[\"My first\", \"sentence pair\"], [\"Second text\", \"pair\"]])",
        "desc": "sentence-transformers 官方文档示例:用 Cross-Encoder 对句子对打分(相关度 0-1)"
      }
    ],
  },
  {
...
<p><strong>代表：</strong>Anthropic Constitutional AI</p>`,
    ...
    quotes: [
      {
        ...
        "cite": "Harrison Lee et al., arXiv:2309.00267 摘要, 2023-09-01"
      },
      {
        "text": "Furthermore, we introduce direct-RLAIF (d-RLAIF) - a technique that circumvents RM training by obtaining rewards directly from an off-the-shelf LLM during RL, which achieves superior performance to canonical RLAIF.",
        "cite": "Harrison Lee et al., arXiv:2309.00267 摘要, 2023-09-01"
      }
    ],
    seeAlso: [
    ...
    examples: [
      ...
      {
        "code": "# RLAIF vs RLHF 的对比引用格式\n# Lee et al. 2023 (arXiv:2309.00267): RLAIF trains the RM on preferences\n#   generated by an off-the-shelf LLM\n# d-RLAIF: rewards obtained directly from an off-the-shelf LLM during RL",
        "desc": "RLAIF 论文两种机制的引用格式(RM 训练式与 direct-RLAIF 直取式)"
      }
    ],
  },
  {
    ...
    quotes: [
      {
        "text": "Building on this foundation, we introduce a straightforward and innovative reference model-free monolithic odds ratio preference optimization algorithm, ORPO, eliminating the necessity for an additional preference alignment phase.",
        "cite": "Jiwoo Hong, Noah Lee, James Thorne, arXiv:2403.07691 摘要, 2024-03-12"
      }
    ],
    seeAlso: [
      {
    ...
    examples: [
      {
        "code": "# ORPO 论文实证结果(引用格式)\n# Phi-2 (2.7B) / Llama-2 (7B) / Mistral (7B) + ORPO on UltraFeedback:\n#   AlpacaEval 2.0 up to 12.20%, IFEval 66.19%, MT-Bench 7.32\n# 超越 >7B/13B 参数量的 SOTA 模型(论文 Figure 1 / Table 6 / Figure 12)",
        "desc": "ORPO 论文的实证结果与数据集引用格式"
      }
    ],
  },
  {
...
<p><strong>优势：</strong>100% 遵循 schema，无需后处理。</p>`,
    ...
    quotes: [
      {
        ...
        "cite": "Brandon T. Willard, Rémi Louf, arXiv:2307.09702 摘要, 2023-07-19"
      }
    ],
    seeAlso: [
      {
      ...
      {
        "name": "Efficient Guided Generation for Large Language Models — arXiv",
        "url": "https://arxiv.org/abs/2307.09702",
        "lastVerified": "2026-09-04"
      },
      {
        "name": "Outlines — dottxt-ai GitHub 仓库",
    ...
    examples: [
      ...
      {
        "code": "# Outlines 官方 README(实抓)对结构化生成的承诺:\n# - Just pass your desired output type: model(prompt, output_type)\n# - Guaranteed valid structure - No more parsing headaches or broken JSON\n# - Provider independence - Switch models without changing code\n# 底层论文: Willard & Louf 2023 (arXiv:2307.09702), finite-state machine 索引法",
        "desc": "Outlines 库对约束解码的产品化表述与底层论文引用格式"
      }
    ],
  },
  {
...
<li><strong>Beam Search</strong>：保留 top-K 序列</li>
    ...
    quotes: [
      {
        "text": "temperature ( float , optional ) — The value used to module the next token probabilities.",
        "cite": "HuggingFace Transformers 文档(GenerationConfig 参数表), 2026-09-04 检索"
      },
      {
        "text": "If set to float < 1, only the smallest set of most probable tokens with probabilities that add up to top_p or higher are kept for generation.",
        "cite": "HuggingFace Transformers 文档(top_p 参数), 2026-09-04 检索"
      }
    ],
    seeAlso: [
      {
      ...
      {
        "name": "Text generation strategies — HuggingFace Transformers 文档",
    ...
    examples: [
      ...
      {
        "code": "# HF Transformers GenerationConfig 采样参数(官方文档实抓):\n# temperature (float, optional) — The value used to module [sic] the next token probabilities.\n# top_k (int, optional) — The number of highest probability vocabulary tokens to keep for top-k-filtering.\n# top_p (float, optional) — If set to float < 1, only the smallest set of most probable\n#   tokens with probabilities that add up to top_p or higher are kept for generation.",
        "desc": "三大采样参数的官方定义引用(temperature/top_k/top_p)"
      }
    ],
  },
  {
...
<li>业务规则散落在 prompt 历史里</li>
</ul><p>Vibe coding 宿醉(vibe coding hangover)指 AI 快速生成代码带来的短期产能快感消退后浮现的维护负担、安全债与可靠性问题。该说法由 Fast Company 2025-09-08 文章《The vibe coding hangover is upon us》定名(正文经 Wayback 快照核验):文中指出许多开发者发现 vibe coding 适合快速拼装 demo,却不适合构建安全、可靠、可解释的软件,且 AI 生成代码的问题可能要到软件上线很久之后才显现;PayPal 高级软件工程师 Jack Zante Hays 直言“AI 编码代理生成的代码可能变成 development hell”。安全厂商 Checkmarx(注意其商业利益)则将宿醉描述为:未经扫描的 AI 生成代码每多积累一周,就在生产率收益之下多沉淀一周的隐性债务。2025-09 上旬的标志性事件是 Coinbase CEO Brian Armstrong 在 X 上宣称其交易所近一半代码已由 AI 生成并计划继续提高比例,原帖随即遭遇大量嘲讽,被视为这一质疑情绪的集中爆发点。</p><p>Vibe coding 宿醉(vibe coding hangover)指 AI 快速生成代码带来的短期产能快感消退后浮现的维护负担、安全债与可靠性问题。该说法由 Fast Company 2025-09-08 文章《The vibe coding hangover is upon us》定名(正文经 Wayback 快照核验):文中指出许多开发者发现 vibe coding 适合快速拼装 demo,却不适合构建安全、可靠、可解释的软件,且 AI 生成代码的问题可能要到软件上线很久之后才显现;PayPal 高级软件工程师 Jack Zante Hays 直言“AI 编码代理生成的代码可能变成 development hell”。安全厂商 Checkmarx(注意其商业利益)则将宿醉描述为:未经扫描的 AI 生成代码每多积累一周,就在生产率收益之下多沉淀一周的隐性债务。2025-09 上旬的标志性事件是 Coinbase CEO Brian Armstrong 在 X 上宣称其交易所近一半代码已由 AI 生成并计划继续提高比例,原帖随即遭遇大量嘲讽,被视为这一质疑情绪的集中爆发点。</p>`,
    enShortDesc: "Fast Company's term (Sep 2025): when vibe-coded apps in production start breaking down.",
    enLongDesc: "<p><strong>Vibe hangover</strong>: the period after vibe coding ships, when the codebase starts breaking down under real use. Maintenance cost exceeds original creation savings.</p><p>The vibe coding hangover refers to the maintenance burden, security debt, and reliability problems that surface once the short-lived productivity high of rapidly AI-generated code wears off. The term was named by Fast Company's 2025-09-08 article \"The vibe coding hangover is upon us\" (text verified via a Wayback snapshot), which observed that many in the development community find vibe coding great for slapping together demos but not so great for building secure, reliable, and explainable software, and that problems created by AI-generated code may only surface long after the software has shipped; Jack Zante Hays, a senior software engineer at PayPal who works on AI software development tools, said that \"Code created by AI coding agents can become development hell.\" Checkmarx, a security vendor (note the commercial interest), frames the hangover as every week of unscanned AI-generated code being another week of debt accumulating quietly underneath the productivity gains. The landmark moment came in early September 2025, when Coinbase CEO Brian Armstrong bragged on X that nearly half his exchange's code was already AI-generated, with plans to push it higher — and the post unleashed a torrent of ridicule.</p><p>The vibe coding hangover refers to the maintenance burden, security debt, and reliability problems that surface once the short-lived productivity high of rapidly AI-generated code wears off. The term was named by Fast Company's 2025-09-08 article \"The vibe coding hangover is upon us\" (text verified via a Wayback snapshot), which observed that many in the development community find vibe coding great for slapping together demos but not so great for building secure, reliable, and explainable software, and that problems created by AI-generated code may only surface long after the software has shipped; Jack Zante Hays, a senior software engineer at PayPal who works on AI software development tools, said that \"Code created by AI coding agents can become development hell.\" Checkmarx, a security vendor (note the commercial interest), frames the hangover as every week of unscanned AI-generated code being another week of debt accumulating quietly underneath the productivity gains. The landmark moment came in early September 2025, when Coinbase CEO Brian Armstrong bragged on X that nearly half his exchange's code was already AI-generated, with plans to push it higher — and the post unleashed a torrent of ridicule.</p>",
    related: ["tech-debt","vibe-slop"],
    quotes: [
      {
        "text": "AI coding assistants Claude Code, Cursor, and Lovable have been riding high, but 'development hell,' 'toxic waste,' and 'evil genies' are killing their buzz.",
        "cite": "Mark Sullivan(文章导语/dek), Fast Company《The vibe coding hangover is upon us》, 2025-09-08, https://www.fastcompany.com/91398622/the-vibe-coding-hangover-is-upon-us (正文取自 Wayback 快照 https://web.archive.org/web/20250908142508/https://www.fastcompany.com/91398622/the-vibe-coding-hangover-is-upon-us, via Wayback)"
      },
      {
        "text": "\"Code created by AI coding agents can become development hell,\" says Jack Zante Hays, a senior software engineer at PayPal who works on AI software development tools.",
        "cite": "Jack Zante Hays(受访者,PayPal 高级软件工程师), 转述自 Mark Sullivan, Fast Company, 2025-09-08, https://www.fastcompany.com/91398622/the-vibe-coding-hangover-is-upon-us (via Wayback)"
      },
      {
        "text": "The vibe coding hangover is real, and it gets worse the longer it goes untreated. Every week of unscanned AI-generated code is another week of debt accumulating quietly underneath the productivity gains.",
        "cite": "Checkmarx 官方博客(安全厂商)《The Vibe Coding Hangover》, 2026-05-14(页面 datePublished), https://checkmarx.com/blog/the-vibe-coding-hangover/"
      }
    ],
    seeAlso: [
      {
        "name": "The vibe coding hangover is upon us — Mark Sullivan, Fast Company",
        "url": "https://www.fastcompany.com/91398622/the-vibe-coding-hangover-is-upon-us",
        "lastVerified": "2026-09-04"
      },
      {
        "name": "The Vibe Coding Hangover — Checkmarx 官方博客(安全厂商)",
    ...
    examples: [
      {
        "code": "# 2025-09 上旬:Coinbase CEO Brian Armstrong 在 X 上宣称其交易所近一半代码已由 AI 生成,并计划继续提高比例,随即遭遇大量嘲讽,被视为 vibe coding 可靠性质疑情绪的集中爆发点\n# \"Coinbase CEO Brian Armstrong was the recipient of some very bad vibes last week after bragging on X that nearly half his exchange's code is already AI-generated, with plans to push it higher. The post unleashed a torrent of ridicule…\"\n#   —— Mark Sullivan, Fast Company《The vibe coding hangover is upon us》, 2025-09-08(正文经 Wayback 快照核验, via Wayback)",
        "desc": "vibe coding 可靠性质疑情绪的标志性爆发点:Coinbase CEO Brian Armstrong 宣称近一半交易所代码已由 AI 生成并计划继续提高比例,原帖遭遇大量嘲讽(Fast Company 2025-09-08 报道,当时被多家媒体报道,可交叉核验)。"
      }
    ],
  },
  {
...
<p><strong>与 vibe coding 关系：</strong>跨域类比——重速度、轻质量；类似"vibe slop"在投资领域。</p><p>“Vibe 估值”(vibe valuation)指风投对 AI 初创公司的巨额估值主要建立在叙事与“感觉”之上,而无视年度经常性收入(ARR)等老派电子表格指标。术语出自《经济学人》2025-06-25 文章《AI valuations are verging on the unhinged》(印刷版标题“Vibe valuations”):其导语(经 Wayback 快照逐字核验,付费墙内正文未引)称 vibe coding 在硅谷大行其道,而它还有个“继兄妹”——“vibe valuing”,即风投“为 AI 初创召唤出巨额估值,而对老派电子表格指标毫不在意”;据 Wikipedia《Vibe coding》词条概括,单数形式“vibe valuation”即经该词条概括传播开来。2025-2026 年该现象在 vibe-coding 赛道集中显现:Lovable 以 133 亿美元估值完成 Series C(CIO.com,2026-08-12;注:文末披露 Lovable 新投资人 Regent 同时是 CIO.com 母公司 Foundry 的所有者),Replit 于 2026-03 达 90 亿美元估值。</p><p>“Vibe 估值”(vibe valuation)指风投对 AI 初创公司的巨额估值主要建立在叙事与“感觉”之上,而无视年度经常性收入(ARR)等老派电子表格指标。术语出自《经济学人》2025-06-25 文章《AI valuations are verging on the unhinged》(印刷版标题“Vibe valuations”):其导语(经 Wayback 快照逐字核验,付费墙内正文未引)称 vibe coding 在硅谷大行其道,而它还有个“继兄妹”——“vibe valuing”,即风投“为 AI 初创召唤出巨额估值,而对老派电子表格指标毫不在意”;据 Wikipedia《Vibe coding》词条概括,单数形式“vibe valuation”即经该词条概括传播开来。2025-2026 年该现象在 vibe-coding 赛道集中显现:Lovable 以 133 亿美元估值完成 Series C(CIO.com,2026-08-12;注:文末披露 Lovable 新投资人 Regent 同时是 CIO.com 母公司 Foundry 的所有者),Replit 于 2026-03 达 90 亿美元估值。</p>`,
    enShortDesc: "AI startup valuations divorced from ARR and fundamentals. The Economist coined this term.",
    enLongDesc: "<p><strong>Vibe valuation</strong>: the trend of AI startups being valued at multiples disconnected from actual revenue. Vibes-based investing.</p><p>\"Vibe valuation\" describes very large AI-startup valuations built by venture capitalists on narrative and feel rather than on old-school spreadsheet fundamentals such as annual recurring revenue. The term traces to The Economist's 2025-06-25 article \"AI valuations are verging on the unhinged\" (print-edition title \"Vibe valuations\"): its lede, verified verbatim via a Wayback snapshot (the paywalled body was not quoted), calls vibe valuing the step-sibling of vibe coding — the ability of venture capitalists to conjure up vast valuations for AI startups with scant regard for old-school spreadsheet measures; per Wikipedia's \"Vibe coding\" entry, the singular form \"vibe valuation\" spread through that summary. The phenomenon was on full display across the vibe-coding ecosystem in 2025-2026: Lovable raised its Series C at a $13.3B valuation (CIO.com, 2026-08-12, which disclosed that Lovable's new investor Regent also owns Foundry, CIO.com's parent company), and Replit reached a $9B valuation in March 2026.</p><p>\"Vibe valuation\" describes very large AI-startup valuations built by venture capitalists on narrative and feel rather than on old-school spreadsheet fundamentals such as annual recurring revenue. The term traces to The Economist's 2025-06-25 article \"AI valuations are verging on the unhinged\" (print-edition title \"Vibe valuations\"): its lede, verified verbatim via a Wayback snapshot (the paywalled body was not quoted), calls vibe valuing the step-sibling of vibe coding — the ability of venture capitalists to conjure up vast valuations for AI startups with scant regard for old-school spreadsheet measures; per Wikipedia's \"Vibe coding\" entry, the singular form \"vibe valuation\" spread through that summary. The phenomenon was on full display across the vibe-coding ecosystem in 2025-2026: Lovable raised its Series C at a $13.3B valuation (CIO.com, 2026-08-12, which disclosed that Lovable's new investor Regent also owns Foundry, CIO.com's parent company), and Replit reached a $9B valuation in March 2026.</p>",
    related: ["vibe-slop"],
    quotes: [
      {
        "text": "Vibe coding, or the ability to spin up a piece of software using generative artificial intelligence (AI) rather than old-school programming skills, is all the rage in Silicon Valley. But it has a step-sibling. Call it vibe valuing. This is the ability of venture capitalists to conjure up vast valuations for AI startups with scant regard for old-school spreadsheet measures.",
        "cite": "The Economist《AI valuations are verging on the unhinged》, 2025-06-25(线上)/ 2025-06-28(印刷版,栏目标题 \"Vibe valuations\"), https://www.economist.com/business/2025/06/25/ai-valuations-are-verging-on-the-unhinged (导语取自 Wayback 快照, via Wayback)"
      }
    ],
    seeAlso: [
      {
        "name": "AI valuations are verging on the unhinged — The Economist",
        "url": "https://www.economist.com/business/2025/06/25/ai-valuations-are-verging-on-the-unhinged",
        "lastVerified": "2026-09-04"
      },
      {
        "name": "What vibe-coding startup valuations portend for CIOs — CIO.com",
        "url": "https://www.cio.com/article/4208833/what-vibe-coding-valuations-portend-for-cios.html",
        "lastVerified": "2026-09-04"
      }
    ],
    examples: [
      {
        "code": "# Vibe-coding 赛道估值案例(2025-2026,数字与表述严格照研究文件/CIO.com 2026-08-12 报道)\n# Lovable(瑞典公司):Series C 融资,估值 $13.3B(133 亿美元)\n# Replit:2026-03 估值达 $9B(90 亿美元)\n# 利益披露:CIO.com 文末披露 Lovable 新投资人 Regent 同时是 CIO.com 母公司 Foundry 的所有者",
        "desc": "vibe 估值狂欢的代表性数字:瑞典公司 Lovable 以 133 亿美元($13.3B)估值完成 Series C(CIO.com,2026-08-12);Replit 于 2026-03 达 90 亿美元($9B)估值。引用 CIO.com 数据需带其利益披露注记。"
      }
    ],
  },
  {
...
<p><strong>vibe coding 影响：</strong>大量"prompt-only"的 PR 涌入开源项目。</p><p>2026 年该词被重新激活:GitHub 官方博客称开源生态正经历 “the Eternal September of open source”——AI 让创建成本骤降而审阅成本未降;Software Freedom Conservancy 进一步提出 AI 时代变体 “Eternal November”(2025 年 11 月,Opus 4.5 发布月)。</p><p>2026 年该词被重新激活:GitHub 官方博客称开源生态正经历 “the Eternal September of open source”——AI 让创建成本骤降而审阅成本未降;Software Freedom Conservancy 进一步提出 AI 时代变体 “Eternal November”(2025 年 11 月,Opus 4.5 发布月)。</p>`,
    enShortDesc: "Usenet term borrowed for GitHub: a flood of newcomers unfamiliar with norms.",
    enLongDesc: "<p><strong>Eternal September</strong>: the phenomenon where a community is overwhelmed by new members who don't know the established norms. Applied to GitHub after AI coding agents flooded it.</p><p>In 2026 the term was revived: The GitHub Blog described open source as hitting \"the Eternal September of open source\" — the cost to create has dropped while the cost to review has not; the Software Freedom Conservancy proposed an AI-era variant, \"Eternal November\" (November 2025, the month of Opus 4.5's release).</p><p>In 2026 the term was revived: The GitHub Blog described open source as hitting \"the Eternal September of open source\" — the cost to create has dropped while the cost to review has not; the Software Freedom Conservancy proposed an AI-era variant, \"Eternal November\" (November 2025, the month of Opus 4.5's release).</p>",
    related: ["vibe-slop"],
    quotes: [
      {
      ...
      {
        "text": "Open source is hitting an “Eternal September.” As contribution friction drops, maintainers are adapting with new trust signals, triage approaches, and community-led solutions.",
        "cite": "Ashley Wolf, The GitHub Blog, 2026-02-12"
      },
      {
        "text": "Today, a pull request can be generated in seconds. Generative AI makes it easy for people to produce code, issues, or security reports at scale. The cost to create has dropped but the cost to review has not.",
        "cite": "Ashley Wolf, The GitHub Blog, 2026-02-12"
      },
      {
        "text": "November 2025 — the month of Opus 4.5’s release — has been widely identified as a similar inflection point: LLM-backed generative AI coding tools made a substantial leap to increased usefulness.",
        "cite": "Denver Gingerich, Software Freedom Conservancy, 2026-04-15"
      }
    ],
    seeAlso: [
      {
      ...
      {
        "name": "Welcome to the Eternal September of open source - The GitHub Blog",
      ...
      {
        "name": "Eternal November — this new influx of users may be better than the last one - Software Freedom Conservancy",
    ...
    examples: [
      {
        "code": "# AI 时代的 \"Eternal September\" 再定义(引用格式)\n# GitHub 官方博客(2026-02-12): 开源生态正经历 \"the Eternal September of open source\"\n#   —— \"The cost to create has dropped but the cost to review has not.\"\n# Software Freedom Conservancy(2026-04-15)提出 AI 时代变体 \"Eternal November\"(2025-11,Opus 4.5 发布月)",
        "desc": "AI 时代该词的机构级再定义引用方式:GitHub 官方博客与 SFC 的两处一手表述"
      }
    ],
  },
  {
...
<p>CoT 的扩展——生成多条推理路径，选最一致的答案。</p>`,
    ...
    quotes: [
      {
        "text": "In this paper, we propose a new decoding strategy, self-consistency, to replace the naive greedy decoding used in chain-of-thought prompting. It first samples a diverse set of reasoning paths instead of only taking the greedy one, and then selects the most consistent answer by marginalizing out the sampled reasoning paths.",
        "cite": "Xuezhi Wang, Jason Wei, Dale Schuurmans et al., arXiv:2203.11171 摘要, 2022-03-21"
      }
    ],
    seeAlso: [
      {
        "name": "Self-Consistency Improves Chain of Thought Reasoning in Language Models — arXiv",
    ...
    examples: [
      ...
      {
        "code": "# Self-Consistency 论文实证数据(引用格式,摘要逐字):\n# GSM8K (+17.9%), SVAMP (+11.0%), AQuA (+12.2%), StrategyQA (+6.4%), ARC-challenge (+3.9%)",
        "desc": "self-consistency 相对贪心解码的基准提升数据(论文摘要)"
      }
    ],
  },
  {
